...
const CashierDetailOrder = ({ route, navigation }) => {
    ...
    const bottomSheetBillRef = useRef(null);
    const totalBayar = item.order_detail.reduce(
    ...
    //     openBillModal()
    //     return () => {
    //         bottomSheetBillRef.current?.dismiss();
    //     }
    // }, [item])
    ...
    const openBillModal = () => {
        bottomSheetBillRef.current?.present();
    }
    ...
    const handleSuccessQris = (response) => {
        closeModal();
        openBillModal()
        ...
        navigation.navigate('SuccessPaymentQris', params);
    ...
    const FormComponentCash = () => {
        ...
        const debounceKembalian = useDebounce(tempKembalian, 100);
        ...
        const handleProcessPaymentCash = () => {
            if (form.jumlah_diberikan < totalBayar) {
                ToastAndroid.showWithGravity(
                    `Proses tidak dapat dilakukan, Uang yang harus dibayar adalah ${formatCurrency(
                        totalBayar,
                        "IDR"
                    )}`,
                    ToastAndroid.SHORT,
        ...
        const isFailedJumlahBayar = useCallback(
            debounce((val) => {
                if (val < totalBayar) {
                    ToastAndroid.showWithGravity(
                        `Uang yang harus dibayar adalah ${formatCurrency(
                            totalBayar,
                            "IDR"
                        )}`,
                        ToastAndroid.SHORT,
            ...
            >
                ...
                <CurrencyInput
                    ...
                    renderTextInput={(textInputProps) => (
                        <TextInput {...textInputProps} variant="filled" />
                    )}
                    prefix="Rp "
                    delimiter="."
                    precision={0}
                ...
                <TouchableOpacity
                    ...
                    onPress={handleProcessPaymentCash}
                >
                ...
                <View style={styles.qrContainer}>
                    ...
                    {expiryTime && (
                        <>
                            ...
                            <EventStatusChecker
                                apiUrl={apiUrl}
                                handleSuccess={handleSuccessQris}
                            />
                        </>
                    )}
                </View>
    ...
    return (
        <View style={styles.ScreenContainer}>
            ...
            {/* {showAnimation && (
        <PopUpAnimation style={styles.LottieAnimation} source={IlSuccesFully} />
      )} */}
            ...
            <View style={[styles.orderContainer, { marginBottom: 2 }]}>
                ...
                <CashierPaymentFooter
                    ...
                    buttonTitle="Bayar"
                    price={{
                        totalPesanan: item.order_detail.reduce(
            ...
            {/* BOTTOMSHEET SUCCESS QRIS */}
                {/* <BottomSheetCustom
                    ref={bottomSheetBillRef}
                    onClose={closeBillModal}
                    disappearsOnIndex={-1}
                >
                    <SuccessPayment navigation={navigation} data={refDataQris.current}/>
                </BottomSheetCustom> */}
        </View>
    );
};
